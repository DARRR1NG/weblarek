...
events.on("contacts:click", () => {
  ...
  const dataAll: IOrderResponse = {
    ...dataBuyerInfo,
    total: basketModel.getAllMoneyBasket(),
    items: basketModel.getListProducts().map((item) => item.id),
  };
  apiModel
    .postData(dataAll)
    .then(() => {
      ...
      modal.render({ content: orderSuccessModel.render() }); // вот тут
    })
    .catch(() => {
      console.error("Ошибка данных");
    });
